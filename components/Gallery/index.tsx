...
export default function Gallery({ images }: GalleryProps) {
  ...
  const getPositionClass = (index: number) => {
    ...
    if (diff === -1 || diff === images.length - 1)
      return {
        container: 'z-20 -translate-x-[40%] lg:-translate-x-[70%] scale-75',
        image: 'brightness-[0.5]',
      }
    if (diff === -2 || diff === images.length - 2)
      return {
        container: 'z-10 -translate-x-[70%] lg:-translate-x-[120%] scale-[0.55]',
  ...
  return (
    <div className="relative overflow-hidden h-[240px] lg:h-[532px] w-full">
      <div {...handlers} className="relative flex h-full items-center justify-center">
        {images.map((img, index) => (
          <div
            key={index}
            className={`absolute flex h-full w-[180px] lg:w-[390px] flex-col items-center justify-center overflow-hidden rounded-[5px] lg:rounded-[20px] text-white transition-all duration-500 ${
